...
public class RestClient {
	...
	public Object sendRequest() throws Exception {
		...
		logger.info(rpcId+"请求URL】:"+this.uri);
		if(CollectionUtils.isNotEmpty(nvps)){
			logger.info(rpcId+"请求参数】:" + mapper.writeValueAsString(this.nvps));
		}
		if(requestBody!=null){
			logger.info(rpcId+"请求JOSN】:" + mapper.writeValueAsString(this.requestBody));
		}
		String json = response(httpClient);
		logger.info(rpcId+"响应结果】:"+json);
